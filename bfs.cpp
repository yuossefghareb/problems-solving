...
int maxdepth(int node ,int prv)
{
    int h=0;
    for(int child:adj[node])if(child!=prv&&!visited[child])
    {
        h=max(h,1+maxdepth(child,node));
    }
    return h;
}
